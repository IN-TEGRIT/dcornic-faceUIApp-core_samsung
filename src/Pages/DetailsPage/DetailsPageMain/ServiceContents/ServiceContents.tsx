...
export default function ServiceContents() {
  ...
  const renderList = () => {
    ...
    if (FILTER_ITEMS[selectedItem].filter === 'all') {
      const Info_List = getFilteredList("Info");
      const Lobby_List = getFilteredList("Lobby");
      return <>
        ...
        <div className="cat-title small">
          <div className="title">안내데스크</div>
          <div className="play-area" data-target="안내데스크" onClick={handleClickPlay}>
            <img src={icPlay} /><label>시작</label>
          </div>
        </div>
        {getListItemComps(Info_List)}

        <div className="cat-title small">
          <div className="title">로비</div>
          <div className="play-area" data-target="로비" onClick={handleClickPlay}>
            <img src={icPlay} /><label>시작</label>
          </div>
        </div>
        {getListItemComps(Lobby_List)}
      </>
    } else {
      const list = getFilteredList(FILTER_ITEMS[selectedItem].filter);
      return (
